...
//	////////////
	...
	public static Response GETPatientbyID(String scenarioName, int patientID) {
		PatientTestData patientTestData = getTestDataForScenario(scenarioName);
		RequestSpecification getpatientbasereq = requestSpecification
				.spec(addAuthentication(scenarioName, patientTestData.getAuthType()))
				.contentType(patientTestData.getContentType());
		response = given().log().all().spec(getpatientbasereq)
				.when()
				.get(replacePatientId(patientTestData.getEndPointURL(),
						patientID + ""))
				.then().log().all().extract().response();
		return response;
	}
//////////////////
	...
	public static Response DeletePatientbyID(String scenarioName, int patientID) {
		PatientTestData patientTestData = getTestDataForScenario(scenarioName);
		RequestSpecification getpatientbasereq = requestSpecification
				.spec(addAuthentication(scenarioName, patientTestData.getAuthType()))
				.contentType(patientTestData.getContentType());
		response = given().log().all().spec(getpatientbasereq)
				.when()
				.delete(replacePatientId(patientTestData.getEndPointURL(),
						patientID + ""))
				.then().log().all().extract().response();
		return response;
	}
/////////////////
	...
	RequestSpecification createPatientBaseReq = requestSpecification
			.spec(addAuthentication(scenarioName, patientTestData.getAuthType())).contentType("application/json"); // Invalid
																													// for
																													// multipart/form-data
	...
	return regAddPatient.when().post(ApiEndpoints.PATIENT_API_PATH.getResources()).then().log().all().extract()
			.response();
}
	...
	public static Response createPatientWithInvalidEndpoint(String scenarioName) {
		...
		List<PatientTestData> filteredPatients = allPatients.stream()
				.filter(d -> scenarioName.equalsIgnoreCase(d.getScenarioName())).collect(Collectors.toList());
		...
		RequestSpecification request = RestAssured.given().baseUri(ConfigReader.getProperty("BaseURL"))
				.basePath("/invalid/api/patient").contentType(ContentType.JSON)
				.header("Authorization", "Bearer " + tokenManager.getAdminToken()).body(patientData);

		// LoggerLoad.info("Sending POST request to invalid endpoint with valid
		// data...");
